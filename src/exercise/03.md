...
Each time when writing JSX, visualize it in React.createElement().
...
Just as we can't use statements as function arguments, we can't use statements
inside of curly braces in JSX. Why? Because, as said above, JSX compiles to
React.createElement() calls, which are function calls.

When spreading ...props and adding additional, explicit, props, the order
matters. Just as the order matters when we spread an object in JavaScript.
