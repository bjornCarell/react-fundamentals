...
Controlled Form inputs. Instead of prompting the user with errors we simply do
the changes needed ourselves to the form input. We can do so by using the value
prop in the form input and assign a handleChange to the onChange prop, in the
handleChange we control the input.
